...
package io.ballerina.lib.ai.googleapis.vertex.compiler;
 ...
 * the Vertex AI model provider's {@code generate} method calls.
 * This enables compile-time schema generation for the {@code generate} API.
 *
 * @since 1.0.0
 */
class GenerateMethodModificationTask implements ModifierTask<SourceModifierContext> {
    ...
    private static final String VERTEXAI_MODEL_PROVIDER_NAME = "ModelProvider";
    private static final String VERTEXAI_MODEL_PROVIDER_MODULE_NAME = "ai.googleapis.vertex";
    private static final String VERTEXAI_MODEL_PROVIDER_MODULE_VERSION = "1";
    private static final String VERTEXAI_MODEL_PROVIDER_MODULE_ORG = "ballerinax";
    private final AiVertexAICodeModifier.AnalysisData analysisData;
    private final ModifierData modifierData;

    public GenerateMethodModificationTask(AiVertexAICodeModifier.AnalysisData analysisData) {
    ...
    public void modify(SourceModifierContext modifierContext) {
        ...
        for (ModuleId moduleId : currentPackage.moduleIds()) {
            ...
            Optional<Symbol> vertexAiModelProviderSymbol =
                    types.getTypeByName(VERTEXAI_MODEL_PROVIDER_MODULE_ORG, VERTEXAI_MODEL_PROVIDER_MODULE_NAME,
                            VERTEXAI_MODEL_PROVIDER_MODULE_VERSION, VERTEXAI_MODEL_PROVIDER_NAME);

            for (DocumentId documentId : documentIds) {
                analyzeDocument(module, documentId, semanticModel, vertexAiModelProviderSymbol);
            }
    ...
    private void analyzeDocument(Module module, DocumentId documentId, SemanticModel semanticModel,
        ...
        analyzeGenerateMethod(semanticModel, modulePartNode, vertexAiModelProviderSymbol, this.analysisData);
    }
    ...
    private void analyzeGenerateMethod(SemanticModel semanticModel,
                                       ...
                                       Optional<Symbol> vertexAiModelProviderSymbol,
                                       AiVertexAICodeModifier.AnalysisData analysisData) {
        new GenerateMethodJsonSchemaGenerator(semanticModel, vertexAiModelProviderSymbol, analysisData)
    ...
    private class GenerateMethodJsonSchemaGenerator extends NodeVisitor {
        ...
        private final ClassSymbol vertexAiProviderSymbol;

        public GenerateMethodJsonSchemaGenerator(SemanticModel semanticModel,
                                                 Optional<Symbol> vertexAiModelProviderSymbolOpt,
                                                 AiVertexAICodeModifier.AnalysisData analyserData) {
            this.semanticModel = semanticModel;
            this.typeMapper = analyserData.typeMapper;
            if (vertexAiModelProviderSymbolOpt.isEmpty()) {
                this.vertexAiProviderSymbol = null;
                return;
            }

            Symbol vertexAiModelProviderSymbol = vertexAiModelProviderSymbolOpt.get();
            if (vertexAiModelProviderSymbol instanceof ClassSymbol vertexAiModelProviderClassSymbol) {
                this.vertexAiProviderSymbol = vertexAiModelProviderClassSymbol;
            } else {
                this.vertexAiProviderSymbol = null;
            }
        }
        ...
        void generate(ModulePartNode modulePartNode) {
            if (this.vertexAiProviderSymbol == null) {
                return;
            }
        ...
        public void visit(RemoteMethodCallActionNode remoteMethodCallActionNode) {
            ...
            semanticModel.typeOf(expression).ifPresent(expressionTypeSymbol -> {
                if (expressionTypeSymbol.subtypeOf(this.vertexAiProviderSymbol)) {
    ...
    private static class TypeDefinitionModifier extends TreeModifier {
        ...
        public TypeDefinitionNode transform(TypeDefinitionNode typeDefinitionNode) {
            ...
            MetadataNode updatedMetadataNode = updateMetadata(typeDefinitionNode, typeSchemas.get(typeName));
