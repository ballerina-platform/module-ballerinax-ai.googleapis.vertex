...
/**
 * Native implementation of Vertex AI helper functions for JSON schema generation
...
public class Native {
    ...
    /**
     * Generates a JSON schema for the given type descriptor at runtime.
     * Returns {@code null} when compile-time schema generation was used (i.e., a
     * {@code @ai:JsonSchema} annotation is already present), so the Ballerina side
     * falls back to the annotation value.
     *
     * @param td the type descriptor to generate a schema for
     * @return a {@code BMap} representing the JSON schema, or {@code null}, or a {@code BError}
     */
    ...
    /**
     * Returns a typedesc for the element type of the given array typedesc.
     *
     * @param expectedResponseTypedesc a typedesc whose described type is an array
    ...
    public static BTypedesc getArrayMemberType(BTypedesc expectedResponseTypedesc) {
        return ValueCreator.createTypedescValue(
                ((ArrayType) TypeUtils.getImpliedType(
                        expectedResponseTypedesc.getDescribingType())).getElementType());
    }

    /**
     * Returns {@code true} when the described type is nilable (i.e., includes {@code null}).
     *
     * @param expectedResponseTypedesc the type descriptor to check
     * @return whether the type is nilable
     */
